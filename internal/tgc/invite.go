...
func (c *Cluster) JoinChannel(ctx context.Context, joiner *Manager, channel database.Channel) error {
	...
	if joiner.ID() == primary.ID() {
		// The primary created or picked the channel, so it is already in it.
		// Recording its access hash is all that is left.
		return c.db.UpsertChannelAccess(ctx, channel.ID, joiner.ID(), channel.AccessHash, true)
	}
	if !joiner.Ready() {
		return fmt.Errorf("%w: sign this account in before adding it to the channel", ErrNotReady)
	}

	link, err := primary.exportInvite(ctx, channel)
	if err != nil {
		return err
	}
	accessHash, err := joiner.joinByInvite(ctx, link, channel)
	if err != nil {
		return err
	}
	...
	if err := primary.promote(ctx, channel, self.GetID()); err != nil {
		// The account is in the channel but cannot post. Record that rather
		// than failing outright: a read-only member is still worth remembering,
		// and the accounts list shows it as unable to store.
		if recErr := c.db.UpsertChannelAccess(ctx, channel.ID, joiner.ID(), accessHash, false); recErr != nil {
			c.log.Warn("could not record a channel membership", zap.Error(recErr))
		}
		return err
	}
	return c.db.UpsertChannelAccess(ctx, channel.ID, joiner.ID(), accessHash, true)
}
